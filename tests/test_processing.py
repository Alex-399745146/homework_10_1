...
def test_filter_by_state(operation: dict) -> None:
    ...
    assert isinstance(volume_str_date, str), f"Тип данных даты не str: {type(volume_str_date)}"
    ...
    assert isinstance(out_data, list), f"Выход данных не list(): {type(out_data)}"
    ...
    assert (
        names_expected_keys == names_actual_keys
