...
def main():
    ...
    print(
        f"""{head_message_ai} Привет! Добро пожаловать в программу работы с банковскими транзакциями.\n
    ...
    if menu_item == "1":
        transactions = get_info_operations(file_json_path)
    elif menu_item == "2":
    ...
    print(f"\n{head_message_ai}({len(transactions)}) Для обработки выбран {name_file[menu_item]}-файл.\n")

    # Вопрос №2 выбор статуса интересующих операций.

    print(
        f"""{head_message_ai} Выберите статус, по которому необходимо выполнить фильтрацию.\n
        Выберите необходимый пункт меню:
        1. EXECUTED
        2. CANCELED
        3. PENDING
    ...
    print(f'\n{head_message_ai}({len(transactions)}) Операции отфильтрованы по статусу "{flag_status}".\n')

    # Вопрос №3 уточнения выборки операций сортировки.

    print(
        f"""{head_message_ai} Отсортировать операции по дате? Да/Нет\n
        Выберите необходимый пункт меню:
        1. Да
        2. Нет
    ...
    print(
        f"""{head_message_ai} Выводить только рублевые транзакции? Да/Нет\n
        Выберите необходимый пункт меню:
        1. Да
        2. Нет\n"""
    )
    ...
    if answer_currency == "1":
        ...
        print(f"\n{head_message_ai}({len(transactions)}) Произведена фильтрация по валюте.\n")

    else:
        print(f"\n{head_message_ai}({len(transactions)}) Фильтрация по валюте не производилась.")
    ...
    if answer_search == "1":
        ...
        if long_txs == 0:
            print(
                f"""\n{head_message_ai}({long_txs}) Не найдено ни одной транзакции, подходящей
    ...
    # Голова отчёта
    print("+" + "-" * 60 + "+")
    print(f"\n{head_message_ai} Распечатываю итоговый список транзакций...\n")
